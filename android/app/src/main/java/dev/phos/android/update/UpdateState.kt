 ...
 * client must never hardcode `/phos.apk`, and the digest has to come from the same
 * response that named the version, or "verified" would only mean "downloaded whatever
...
sealed interface UpdateState {
    ...
    data class Available(
        ...
        val versionCode: Int,
        val sizeBytes: Long,
        /** Digest the downloaded file must hash to before it is allowed near the installer. */
        val sha256: String,
        /** Server-supplied download location, so the APK path lives in exactly one place. */
        val downloadPath: String,
    ) : UpdateState

    /** The check itself failed. Carries a sentence naming what went wrong. */
...
object UpdateCheck {
    ...
    fun decide(running: RunningVersion, advertised: ServerBuild?): UpdateState {
        ...
        // download, "verify before install" has nothing to verify against. Never
        // reported as UpToDate, which would hide a broken release pipeline behind a
