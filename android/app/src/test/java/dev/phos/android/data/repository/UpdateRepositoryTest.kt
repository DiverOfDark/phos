...
import dev.phos.android.update.AppIdentity
import dev.phos.android.update.FakeSignatures
...
class UpdateRepositoryTest {
    ...
    init {
        every { auth.getServerUrl() } returns "https://phos.example.com"
    }
    ...
    private fun repository(
        ...
        archivePackage: String = "dev.phos.android",
    ): Pair<UpdateRepository, FakeInstaller> {
        val signatures = FakeSignatures(
            running = AppIdentity("dev.phos.android", 42, ourSigner),
    ...
    private fun file(bytes: ByteArray): File =
        File(temp.root, "phos-43.apk").apply { writeBytes(bytes) }
    ...
    fun `a strictly newer server build becomes Available`() = runTest {
        ...
        assertEquals("/phos.apk", available.downloadPath)
