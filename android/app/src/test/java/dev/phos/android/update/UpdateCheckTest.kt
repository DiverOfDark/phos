...
class UpdateCheckTest {
    ...
    fun `the local dev build is out-ranked by every real server build`() {
        ...
        val dev = RunningVersion(versionCode = 1, versionName = "1.0.0")
    ...
    @Test
    fun `a newer build with no usable digest is refused rather than offered`() {
        ...
        assertTrue(failed.message.contains("not a digest"))
    }
    ...
    private companion object {
        /** Real 64-hex digest, of the shape the sidecar the Docker build emits carries. */
