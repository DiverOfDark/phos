...
struct Timeouts {
    ...
    queue: Duration,
    /// Whole-call budget for `/object_info`. The answer runs to megabytes on a
    /// loaded box, so it gets more room than the small JSON endpoints.
    catalogue: Duration,
...
impl Default for Timeouts {
    fn default() -> Self {
        Self {
            ...
            catalogue: Duration::from_secs(30),
...
impl ComfyUiClient {
    ...
    /// budget: an import must not hang on a server that accepted the
    ...
    pub(crate) fn object_info(&self) -> anyhow::Result<Value> {
        ...
        let mut resp = ureq::get(&url)
            .config()
            .timeout_global(Some(self.timeouts.catalogue))
            .build()
            .call()
